...
- GIN (Generalized Inverted Index) 인덱스는 복합 데이터 유형에서 효율적인 검색을 위해 설계되었습니다.
  ...
  - 일반적으로 데이터베이스는 B-Tree 인덱스를 사용하여 데이터를 검색합니다.
...
- 텍스트 검색 쿼리:
  ...
  ```sql
  SELECT * FROM documents WHERE to_tsvector('english', content) @@ to_tsquery('text & search');

  ```

- 자주 업데이트되는 테이블에서는 업데이트 성능이 저하될 수 있습니다.
- 그러나 복합 데이터 유형의 쿼리 성능을 크게 향상시킬 수 있습니다.
